...
triesite::triesite(string name = "", char func = 'c', char mode = 'q') {
	if((name[1]==':' && name[2]=='\\') || (name.compare("") == 0)) {
		sitename = name;
	} else {
		sitename = getCurrentPath();
		sitename += ("\\" + name);
	}
	if(func == 'C' || func == 'c'){
	create(sitename);
...
void triesite::create(string path){
	doclist.clear();
	int err = createDirectory(path.c_str());
	if(err != 0){
		throw new exception("Unable to create a search site");
	}
}

void triesite::mount(string path,char mode='q'){
